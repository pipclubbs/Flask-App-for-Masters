...
'''
    def __str__(self):
        return f"{name}, {url}, {title}, {description}"
'''


class NorthEastClasses:
    def __init__(self, name='Climb Valley, Newcastle', url='https://www.climbnewcastle.com/index.php?route=extension/cn/essentials', title='', description=''):
        self.name = name
        self.url = url
        self.title = title
        self.description = description

        # assign_values = self.assign_values()
        # assign_values
    ...
    def assign_values(self):
        database_conn = ''
        # tag_list = self.search_tags()
        output = []
        p_tag_list = []
        valley_classes = []

        h1_tag = self.search_tags('h1')
        h4_tag = self.search_tags('h4')
        p_tag = self.search_tags('p')
        ...
        for i in class_list:
            valley_classes.append(i)

        output.append(db_conn2.DatabaseConnection(valley_classes))
